...
def plot_true_clusters():
  for k in range(K):
    ...
    GaussViz.plotGauss2DContour(mus[k], sigmas[k], color=GaussViz.Colors[c])

def plot_sequence(seqID, Data, dimID=0, maxT=200):
  Xseq = Data.X[Data.doc_range[seqID]:Data.doc_range[seqID+1]]
  Zseq = Data.TrueParams['Z'][Data.doc_range[seqID]:Data.doc_range[seqID+1]]
  ...
  for ii, loc in enumerate(changePts[:-1]):
    ...
    pylab.plot([nextloc-1,nextloc], [Xseq[nextloc-1],Xseq[nextloc]], 'k:')
  pylab.ylim([-2, 14])
