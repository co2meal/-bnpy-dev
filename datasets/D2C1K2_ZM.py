...
# ########################################################## User-facing
def get_data(seed=8, nObsTotal=250000, **kwargs):
...
Psi = np.zeros((K,D,D))
Psi[0] = np.diag([1, 5]) * 10
Psi[1] = np.diag([1, 1]) * 20
...
def plot_data_by_k(Data):
    ...
    for k in range(K):
        ...
        x = Data.X[idx, 0]
        y = Data.X[idx, 1]
        plt.scatter(x, y, c=GaussViz.Colors[c], marker='o')
        # z = Data.X[idx, 2]
...
def plot_W_by_k(hModel, Data):
    from bnpy.viz import GaussViz
    colorset = ['y','g','b']
    WMean = hModel.obsModel.Post.WMean
    LP = hModel.calc_local_params(Data, None)
    for k in range(3):
        ax = plt.axes()
        n = 50 / (np.mean(WMean[k]))
        ax.arrow(0, 0, n * WMean[k,0,0], n * WMean[k,1,0], width=2, head_width=10, head_length=10,
                 fc=colorset[k], ec=colorset[k])
        idx = np.flatnonzero(np.argmax(LP['resp'],axis=1) == k)
        x = Data.X[idx, 0]
        y = Data.X[idx, 1]
        plt.scatter(x, y, c=colorset[k], marker='o')



if __name__ == '__main__':
    Data = get_data(nObsTotal=10000)
    # plot_data_by_k(Data)
    hModel, RInfo = bnpy.run(Data, 'FiniteMixtureModel','ZeroMeanFactorAnalyzer', 'VB',
                             C=1, nLap=100, K=3, printEvery=1)
    plot_W_by_k(hModel, Data)
    plt.show()
