...
########################################################### User-facing 
###########################################################
def get_data(seed=DEFAULT_SEED, seqLen=DEFAULT_LEN, **kwargs):
  ''' Generate toy data sequences, returned as a bnpy data-object
    ...
    seqLen : int number of observations in each sequence

    Returns
    -------
    Data : bnpy GroupXData object, with nObsTotal observations
  '''
  X, Xprev, Z, doc_range = get_X(seed, seqLen)
...
def get_X(seed, seqLen=DEFAULT_LEN):
    '''
    ...
    # 3 sequences, all the same length (=seqLen)
    seqLens = int(seqLen) * np.ones(3, dtype=np.int32)
    ...
    Z1, X1, Xprev1 = generateSequence_ZandX(seqLen, [0, 1, 2], A_master, Sigmas, prng)
    Z2, X2, Xprev2 = generateSequence_ZandX(seqLen, [0, 1, 2], A_master, Sigmas, prng)
    Z3, X3, Xprev3 = generateSequence_ZandX(seqLen/4, [2, 3], A_master, Sigmas, prng)
    ...
    seqLens = [Z1.size, Z2.size, Z3.size]
