'''
OptimizerForHDP3.py
Version 3: avoids unused/leftover theta term,
instead parameterizes everything in terms of leftover beta.
...
def find_optimum(sumLogPi=None, nDoc=0, gamma=1.0, alpha=1.0,
                 initrho=None, initomega=None, thetaVersion=1,
...
def objFunc_constrained(rhoomega,
                     ...
                     approx_grad=False, thetaVersion=1):
  ...
  if nDoc > 0:
    if thetaVersion:
      elbo += gamma * np.inner(Ebeta[:-1], sumLogPi/nDoc)
    else:
  ...
  if nDoc > 0:
    # dBdrho : K x K matrix
    #  dBdrho[j,k] = d/d rho[j] ( Ebeta[k] )
    if thetaVersion:
      dBdrho = np.tile(-1*Ebeta[:-1], (K,1))
    else:
      dBdrho = np.tile(-1*Ebeta, (K,1))
    dBdrho /= (1-rho)[:,np.newaxis]
    diagIDs = np.diag_indices(K)
    dBdrho[diagIDs] /= -1 * rho/(1-rho)
    dBdrho[_get_lowTriIDs(K)] = 0
    gradrho += gamma * np.dot(dBdrho, sumLogPi/nDoc)
    if thetaVersion:
      gradrho -= gamma * Ebeta[-1] / (1-rho) * digamma(gamma*Ebeta[-1])
