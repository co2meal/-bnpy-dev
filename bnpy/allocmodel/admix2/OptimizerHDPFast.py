...
def find_optimum_multiple_tries(DocTopicCount=0, nDoc=0, 
  ...
  for trial, factr in enumerate(factrList):
    try:
      rhoomega, f, Info = find_optimum(DocTopicCount, nDoc,
  ...
  if rhoomega is None:
    if initrho is not None:      
      ...
      return find_optimum_multiple_tries(DocTopicCount, nDoc, 
                                gamma=gamma, alpha=alpha,
                                initrho=None, initomega=None,
...
def find_optimum(DocTopicCount=0, nDoc=0, gamma=1.0, alpha=1.0,
  ...
  if nDoc > 0:
    ...
    D, K = DocTopicCount.shape
    assert D == nDoc
  else:
    K = DocTopicCount.size
...
def objFunc_constrained(rhoomega,
  ...
  if nDoc > 0:
    ...
    elbo_local = cDir_qPi / nDoc + cDirRem_qPi
  ...
  if nDoc > 0:
    ...
    Gvec = np.sum(digamma(theta), axis=0) / nDoc
    Gvec = np.append(Gvec, digamma(alphaEbeta[-1]))
    gradrho += alpha * np.dot(Delta, Gvec)
