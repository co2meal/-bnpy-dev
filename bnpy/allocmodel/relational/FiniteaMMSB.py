...
class FiniteaMMSB(FiniteMMSB):
  ...
  def calc_evidence(self, Data, SS, LP, **kwargs):
    ...
    if SS.hasELBOTerm('Elogqz'):
      ...
      entropy = MMSBUtil.assortative_elbo_entropy(LP, Data, self.epsilon,
                                                  Data.nNodesTotal, self.K,
                                                  self.expELogPi,
                                                  self.sumExpELogPi)
    extraObsModelTerm = MMSBUtil.assortative_elbo_extraObsModel(Data, LP,
                                                                self.epsilon)
  ...
  def elbo_entropy(self, LP, Data):
    ...
    H = (Data.nNodes**2-Data.nNodes-len(Data.edgeSet))*np.log(1-self.epsilon)+\
    ...
    scratch = LP['resp'] * (logSoftEv[0,:] - np.log(1-self.epsilon))[np.newaxis,np.newaxis,:]
    scratch[Data.respInds[:,0],Data.respInds[:,1]] /= \
    ...
    tmp = LP['resp'].copy()
    tmp *=  logSoftEv[0,:]
    tmp[Data.respInds[:,0],Data.respInds[:,1]] *= \
                                      (logSoftEv[1,:]/logSoftEv[0,:])
    scratch = tmp.copy()
    tmp = np.log(1-self.epsilon) * LP['resp']
    tmp[Data.respInds[:,0],Data.respInds[:,1]] *= \
                                  (np.log(self.epsilon)/np.log(1-self.epsilon))
    scratch -= tmp
    term1Alt = np.sum(scratch)
    '''    
...
'''    
array([16, 10, 48, 28, 41, 52, 70, 58])

array([[ 0.73158993,  0.72300943,  0.04857547,  0.73332297],
       [ 0.70402234,  0.72785064,  0.75401788,  0.72631452],
       [ 0.74050947,  0.73196756,  0.71021672,  0.74851834],
       [ 0.75824325,  0.74257282,  0.70742899,  0.03930918],
       [ 0.7275718 ,  0.72354246,  0.72152833,  0.73777827],
       [ 0.04163778,  0.73465471,  0.72181999,  0.7510694 ],
       [ 0.75400637,  0.73089716,  0.70354137,  0.74986919],
       [ 0.03370515,  0.71173506,  0.71511769,  0.72752273]])


ACTUAL = q
array([[ 0.73158993,  0.72300943,  0.03906145,  0.73332297],
       [ 0.70402234,  0.72785064,  0.75401788,  0.72631452],
       [ 0.74050947,  0.73196756,  0.71021672,  0.74851834],
       [ 0.75824325,  0.74257282,  0.70742899,  0.04033812],
       [ 0.7275718 ,  0.72354246,  0.72152833,  0.73777827],
       [ 0.03943759,  0.73465471,  0.72181999,  0.7510694 ],
       [ 0.75400637,  0.73089716,  0.70354137,  0.74986919],
       [ 0.03904586,  0.71173506,  0.71511769,  0.72752273]])
