...
class HDPaMMSB(AllocModel):
  ...
  def __init__(self, inferType, priorDict=dict()):
    ...
    self.omega = None
    self.theta = None #Nx(K+1) Dirichlet params for community distributions pi
  ...
  def set_prior(self, gamma=10, alpha=0.5, epsilon=0.05,
    ...
    # Used for assortativity, delta only for assortative real-valued likelihoods
  ...
  def calc_local_params(self, Data, LP, MergePrepInfo=None, **kwargs):
    ...
    if Data.isSparse:
      ...
      LP = self._calc_local_params_nonsparse(Data, LP, MergePrepInfo, **kwargs)
    return LP
  ...
  def _calc_local_params_sparse(self, Data, LP, MergePrepInfo=None, **kwargs):
    K = self.K
    respI = Data.respInds
  ...
  def _get_global_suff_stats_nonsparse(self, Data, LP):
    ...
    scratch *= \
              (self.sumExpELogPi[:,np.newaxis] - self.expELogPi)[np.newaxis,:,:]
    ...
    # Calculate sumReceiver[i,l] = \sum_j r_{jil} = \sum_j \sum_m resp_{jiml}
    scratch[:,:,:] = (mask*((1-self.epsilon)/(2*self.M)) + 
                      (1-mask)*(self.epsilon/(2*self.M)))[:,:,np.newaxis]
    scratch /= LP['normConst'][:,:,np.newaxis]
    scratch *= \
     (self.sumExpELogPi[inds,np.newaxis]-self.expELogPi[inds,:])[:,np.newaxis,:]
  ...
  def update_global_params_VB(self, SS, **kwargs):
    ...
    self.rho, self.omega = self.find_optimum_rhoOmega(N=SS.nNodes, **kwargs)
    self.theta = self._calcTheta(SS)
  ...
  def evaluateHeldOut(self, Data, SS, obsModel):
    ...
    epi = self.theta[:,:K] / np.sum(self.theta[:,:K], axis=1)[:,np.newaxis]
    epi = epi.squeeze()
    for ee, (i,j) in enumerate(Data.heldOutSet):
      self.edgePrPi[ee,0] = i
      self.edgePrPi[ee,1] = j
      self.edgePrPi[ee,2] = np.sum(epi[i,np.newaxis,:,np.newaxis]*epi[np.newaxis,j,np.newaxis,:]*w)
  ...
  def calcEdgePrPrecisionRecall(self, edgePr, threshs):
    ...
    for tt, thresh in enumerate(threshs):
      ...
      onesRight = np.sum(np.logical_and(scratch == edgePr[:,3], edgePr[:,3] == 1))
      zerosRight=np.sum(np.logical_and(scratch == edgePr[:,3], edgePr[:,3] == 0))
      onesWrong = np.sum(np.logical_and(scratch != edgePr[:,3], edgePr[:,3] == 1))
      zerosWrong=np.sum(np.logical_and(scratch != edgePr[:,3], edgePr[:,3] == 0))
