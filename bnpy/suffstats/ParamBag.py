...
class ParamBag(object):
  ...
  def addDims(self, dimStrs, dimInts):
    ''' Adds dimensions to itself with names dimStrs and of size dimInts
        Useful for adding in dimensions with unusual names (not 'K','N','D') 
    '''
    for dimStr,dimInt in zip(dimStrs,dimInts):
      setattr(self,dimStr,dimInt)
