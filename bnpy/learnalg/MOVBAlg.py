...
class MOVBAlg(LearnAlg):
  ...
  def fit(self, hmodel, DataIterator):
    ...
    while DataIterator.has_next_batch():
      ...
      ## Held-out evaluation
      if self.isSaveDiagnosticsCheckpoint(lapFrac, iterid) and \
         self.isLastBatch(lapFrac) and \
         hasattr(DataIterator.Data, 'heldOut') and \
         DataIterator.Data.heldOut is not None:
        hmodel.allocModel.evaluateHeldOut(DataIterator.Data, SS,
                                          hmodel.obsModel)
        

      ## ELBO calculation
      evBound = hmodel.calc_evidence(SS=SS, Data=Dchunk, LP=LPchunk)
