...
def runDeleteMove_SingleSequence(n, Data, curSS, SS_n, curModel,
    ...
    ''' Evaluate candidate model that removes unique states in sequence n.
        ...
        Plan
    '''
    # TODO : run several  cycles with full model, to be in good start pos??
    ...
    newModel, newSS, Plan = runDeleteMove_Target(
        curModel, curSS, Plan, **kwargs)
    delattr(newSS, 'uIDs')
...
def runDeleteMove_Target(curModel, curSS, Plan,
    ...
    for delCompID in Plan['uIDs']:
        ...
        for riter in xrange(nRefineIters):
            ...
            propELBO = propModel.calc_evidence(
                targetData,
                ptargetSS,
                ptargetLP)
    ...
    if didAccept:
        ...
        for batchID in SSmemory:
            ...
            if 'targetSSByBatch' in Plan and batchID in Plan[
                    'targetSSByBatch']:
