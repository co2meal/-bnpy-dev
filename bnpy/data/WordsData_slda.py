...
class WordsData(DataObj):
    ...
    def add_data(self, WData):
		...
		if hasattr(self,'response'):
			if hasattr(WData, 'response'):
				...
				print 'Warning: response variables not added'
    ...
    def getRawDataAsSharedMemDict(self):
        ...
        if hasattr(self,'response'):
        	dataShMemDict['response'] = numpyToSharedMemArray(self.resopnse)
    ...
    def select_subset_by_mask(self, docMask,
		...
		if hasattr(self,'response'):
			response = np.zeros(nDoc,dtype=self.response.dtype)
		else:
			response = None
		...
		for d in xrange(nDoc):
			...
			if hasattr(self,'response'):
				response[d] = self.response[docMask[d]]
			startLoc += (stop - start)
		...
		return WordsData(word_id, word_count, doc_range, self.vocab_size,
						 response=response,nDocTotal=nDocTotal, TrueParams=newTrueParams)
    ...
    def CreateToyDataSimple(cls, nDoc=10, nUniqueTokensPerDoc=10,
		...
		response = PRNG.choice(5, size=nDoc,replace=True)
    ...
    @classmethod
    def CreateToyDataFromSLDAModel(cls, seed=101,
                                  nDocTotal=None,
                                  nWordsPerDoc=None,
                                  nWordsPerDocFunc=None,
                                  topic_prior=None, topics=None,
