...
class GraphXData(XData):
  ...
  @classmethod
  def LoadFromFile(cls, filepath, nNodesTotal=None, isSparse=False, **kwargs):
  ...
  @classmethod
  def read_from_txt(cls, filepath, isSparse=False, nNodesTotal=None, **kwargs):
  ...
  def holdOutData(self, holdOutFraction):
    ...
    linksInds = np.random.choice(np.arange(len(self.edgeSet)), replace=False,
                                  size=(linksHeldOut,))
    self.heldOut = self.respInds[linksInds,:].T

    # This will actually re-choose *some* links, but we'll still get roughly
    #  holdOutFraction of non-links (assuming the graph is sparse)
    self.heldOut = np.append(self.heldOut,
    ...
    for i in xrange(self.heldOut.shape[1]):
      if self.heldOut[0,i] == self.heldOut[1,i]:
        while self.heldOut[0,i] == self.heldOut[1,i]:
          self.heldOut[:,i] = np.random.randint(low=0, high=self.nNodes,
                                                size=(2,))
    self.heldOutSet = set(zip(self.heldOut[0], self.heldOut[1]))
  ...
  def get_stats_summary(self):
    ...
    if self.isSparse:
      ...
      s = 'Graph with N = %d nodes and real-valued edges of ' % (self.nNodes)
  ...
  def _create_resp_indices(self):
    ...
    self.respIndsSet = set(zip(self.respInds[:,0], self.respInds[:,1]))
    ...
    for i in mask:
      ...
      for j in xrange(self.nNodes):
        ...
        if (self.heldOut is not None) and (i,j) in self.heldOutSet:
          heldOut = np.append(heldOut, np.asarray([[i,j]],dtype=np.int32).T,
                              axis=1)
