...
class iVBLearnAlg( LearnAlg ):
  ...
  #####################################################################
  #####################################################################
  def dec_suff_stats_from_chunk( self, SS, SSchunk, doSkip=False ):
    for key in SS:
      if type( SS[key] ) is not np.ndarray and type( SS[key] ) is not np.float64:
        continue
      if doSkip and (key == 'Hz' or key=='Hmerge'):
        continue
      if SS[key].size >= 1:
        SS[key] -= SSchunk[key]

  def inc_suff_stats_from_chunk( self, SS, SSchunk, doSkip=False ):
    for key in SSchunk:
      if key not in SS:
  ...
  def fit( self, hmodel, DataGenerator, AllData=None ):
    ...
    if hasattr(self, 'moves') and ('M' in self.moves or 'p' in self.moves):
      MoveLog['Mflag'] = True
    else:
      MoveLog['Mflag'] = False
    ...
    iterid = -1
    while DataGenerator.has_next():
      iterid += 1
      Dchunk = DataGenerator.get_next()
      bID = Dchunk['bID']

      if hasattr(self, 'moves'):
        hmodel, SS, MoveLog = self.run_first_moves( hmodel, SS, iterid, Dchunk['passID'], Dchunk['repID'], MoveLog)
      ...
      if 'SSbirth' in MoveLog:
        self.inc_suff_stats_from_chunk( SS, MoveLog['SSbirth'], doSkip=True )
      ...
      if 'SSbirth' in MoveLog:
        self.dec_suff_stats_from_chunk( SS, MoveLog['SSbirth'], doSkip=True )
      ...
      if 'N' in SS:
        Nafter = np.sum( SS['N'] )
        assert np.allclose( Nbefore, Nafter)
        assert np.allclose( SS['Ntotal'], Nafter)
      ############################################################################################################3
      #######################################################################################################3
      #################################################################################################3
      ...
      if hasattr(self, 'moves'):
        self.run_subsampling( Dchunk, LPchunk, MoveLog)
