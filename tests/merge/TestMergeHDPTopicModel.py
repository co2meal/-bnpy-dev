...
class TestMergeHDP(unittest.TestCase):
  ...
  def test_ELBO_Z_terms_are_correct(self):
    ...
    memoElogpZ = np.sum(SS.getPrecompELBOTerm('ElogpZ'))
    memoElogqZ = np.sum(SS.getPrecompELBOTerm('ElogqZ'))
  ...
  def test_ELBO_Z_terms_are_correct_merge(self):
    ...
    memoElogpZ = SS.getPrecompELBOTerm('ElogpZ')
    memoElogqZ = SS.getPrecompELBOTerm('ElogqZ')
    assert np.allclose(directElogpZ, memoElogpZ)
    assert np.allclose(directElogqZ, memoElogqZ)
    ...
    WMat = SS.getPrecompMergeTerm('ElogpZ')
  ...
  def test_ELBO_Pi_terms_are_correct(self):
    ...
    memoElogqPi = np.sum(SS.getPrecompELBOTerm('ElogqPiVec')) \
                    + SS.getPrecompELBOTerm('ElogqPiConst')
  ...
  def test_ELBO_Pi_terms_are_correct_merge(self):
    ...
    SS = SS1 + SS2
    assert SS.nDoc == self.Data.nDocTotal
    cTerm = SS.getPrecompELBOTerm('ElogqPiConst')
    cTerm1 = SS1.getPrecompELBOTerm('ElogqPiConst')
    cTerm2 = SS2.getPrecompELBOTerm('ElogqPiConst')
    assert np.allclose(cTerm, cTerm1 + cTerm2)

    # Consider merge of comps 0 and 1
    K = SS.K
    print "BEFORE sumLogPi", SS.sumLogPi

    kA = 0
    kB = 1
    mLP1 = self.calc_mergeLP(LP1, kA, kB)
    mLP2 = self.calc_mergeLP(LP2, kA, kB)
    mergeSumLogPi_kA = SS.getPrecompMergeTerm('sumLogPi')[kA,kB]

    SS.mergeComponents(kA, kB)
    assert np.allclose(SS.getPrecompELBOTerm('ElogqPiConst'), cTerm1 + cTerm2)

    sumLogPi = mLP1['E_logPi'].sum(axis=0) + mLP2['E_logPi'].sum(axis=0)
    assert sumLogPi[kA] == mergeSumLogPi_kA
    assert np.allclose(sumLogPi, SS.sumLogPi)
