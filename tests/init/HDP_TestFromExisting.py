...
if __name__ == '__main__':
    ...
    Npersymbolperdoc = PRNG.multinomial(Npersymbol, [1.0/Ndoc] * Ndoc, size=10)
    for i in xrange(Ndoc):
        for ii, patch_name in enumerate(['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J']):
            X_ND = S.generate_patches_for_symbol(patch_name, Npersymbolperdoc[ii,i])
            Xlist.append(X_ND)
            Zlist.append(ii * np.ones(X_ND.shape[0], dtype=np.int32))
        doc_range.append(np.sum(Npersymbolperdoc[:, :i+1]))
    X = np.vstack(Xlist)
    TrainData = bnpy.data.GroupXData(X, doc_range=doc_range, TrueZ=np.hstack(Zlist))
    ...
    for patch_name in ['A', 'B', 'C', 'D',
        ...
        X_ND = S.generate_patches_for_symbol(patch_name, NpersymbolTest)
        Xlist.append(X_ND)
    X = np.vstack(Xlist)
    ...
    testSS = trainedModel.get_global_suff_stats(TestData, testLP, doPrecompEntropy=1, doTrackTruncationGrowth=1)
